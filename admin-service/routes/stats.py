...
def overview():
    try:
        r1 = requests.get(f"{ORDERS_URL}/orders/summary", timeout=6)    # aggregate order metrics
        r2 = requests.get(f"{LISTINGS_URL}/posts/summary", timeout=6)   # aggregate listing metrics
        data = {
