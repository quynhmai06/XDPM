...
def checkout_page(payment_id: int):
    ...
    if request.method == "POST":
        ...
        # ✅ NHÉT NGAY TẠI ĐÂY — đổi phương thức nếu người dùng chọn khác
        try:
  ...
  <div class="grid">
    ...
    <div class="col">
      <div class="card">
        ...
        <!-- BUYER INFO (ngay dưới phương thức) -->
        <div class="section">
      ...
      <form id="orderForm" class="card" method="post" action="">
        ...
        <div class="summary">
          ...
          <div class="row" style="justify-content:space-between;margin-top:6px">
            ...
            <span style="color:#16a3 4a">Miễn phí</span>
          </div>
          <div class="hr"></div>
          <div class="total">
...
def confirm_payment(payment_id: int):
    ...
    # ⚠️ NỚI LỎNG: chỉ bắt buộc full_name, phone
    if not invoice:
...
# POST: API ghi chữ ký (buyer/seller) – endpoint tường minh, giữ nguyên logic hiện tại
...
def invoice_page(contract_id: int):
    ...
    buyer_name = info.get("full_name") or getattr(payment, "buyer_name", None) or "Khách hàng"
  ...
  <div style="border:1px solid var(--line);border-top:none;border-radius:0 0 12px 12px;padding:12px 14px;background:#fff">
    ...
    {% if sale_id %}
      <div style="display:flex;gap:10px;flex-wrap:wrap">
        <!-- Chỉ còn nút xem hợp đồng (mở popup) -->
...
<div id="contractModal" class="modal-backdrop">
  <div class="modal" role="dialog" aria-modal="true" aria-label="Nội dung hợp đồng">
    ...
    <div class="modal-body">
      <!-- Trình bày hợp đồng cân đối, chuyên nghiệp -->
    ...
    <div class="modal-foot">
      {% if sale_id %}
        {% if buyer_signed %}
          <!-- Người mua đã ký: nút mờ và không bấm được -->
...
// Hàm kiểm tra trạng thái và redirect nếu đã thanh toán
...
// Hàm gọi khi bấm nút “Đã chuyển tiền – kiểm tra”
async function checkStatus(){
  const ok = await _checkAndMaybeRedirect();
  if (ok) return;

  // Nếu chưa thanh toán thì hiển thị trạng thái và bắt đầu poll 5s/lần
...
def _build_checkout_ui(payment: Payment):
    # Lấy tên sản phẩm từ query nếu có (gateway có thể truyền ?product_name=...)
    product_name = request.args.get("product_name") or request.args.get("name") or ""
    # Ảnh sản phẩm (nếu gateway truyền ?img=...)
    product_img = request.args.get("img") or ""
    # Địa phương (nếu cần hiển thị)
    province = request.args.get("province") or ""

    subtotal = float(payment.amount or 0)
    shipping = 0.0
    # Ở màn checkout tổng = tạm tính + phí ship (giữ giống hình => không cộng VAT tại bước này)
    total = subtotal + shipping

    return {
        "product_name": product_name or f"Đơn hàng #{payment.order_id}",
        "product_img": product_img,   # có thì hiện, không thì ẩn
        "province": province,
        "qty": 1,
        "subtotal": subtotal,
        "shipping": shipping,
        "total": total,
        "methods": [
            {"key": "banking", "label": "Chuyển khoản ngân hàng", "desc": "Chuyển khoản qua Internet Banking hoặc QR Code", "icon": "🏦"},
            # Nếu muốn thêm sau: {"key":"e_wallet","label":"Ví điện tử","desc":"MoMo/ZaloPay...", "icon":"💳"},
        ],
        "current_method": payment.method.value,
    }

# GET: trang UI “Ký ngay” (KH xem nội dung & ký)
...
<script>
  // nếu đã ký thì chặn JS luôn
  const already = {{ 'true' if buyer_already_signed else 'false' }};
  if (already) {
    document.getElementById('msg').textContent = 'Bạn đã ký hợp đồng này. Nút ký được khóa.';
  }
</script>
...
"""
    # Tìm invoice liên quan (để nút quay lại hoá đơn)
